...
RUBY_SCRIPT="r=Ci::Runner.new;r.runner_type='instance_type';r.description='$RUNNER_DESCRIPTION';r.tag_list=['docker','shadow-file'];r.run_untagged=true;r.locked=false;r.access_level='not_protected';r.save!;print r.token"

# Use docker compose exec with service name
RUNNER_TOKEN=$(docker compose exec $GITLAB_SERVER_CONTAINER gitlab-rails runner "$RUBY_SCRIPT" 2>/dev/null)
...
# Resolve the actual container ID for the gitlab-server service
...
# Inspect the gitlab-server container ID to find which network it is attached to.
